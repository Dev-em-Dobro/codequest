...
// Security: Sanitize inputs to prevent AI injection attacks
function sanitizeInput(input: string): string {
  if (!input || typeof input !== 'string') {
    return '';
  }
  
  // Remove potential injection patterns and limit size
  return input
    .replace(/[<>\"\'`]/g, '') // Remove HTML/JS injection chars
    .replace(/\{[^}]*system[^}]*\}/gi, '') // Remove potential system role injections
    .replace(/\{[^}]*role[^}]*\}/gi, '') // Remove role injections
    .replace(/```[^`]*```/g, '') // Remove code blocks that could contain injections
...
function sanitizeCode(code: string): string {
  ...
  // Allow normal code characters but limit size and remove dangerous patterns
  return code
    .replace(/eval\s*\(/gi, 'EVAL_BLOCKED(') // Block eval attempts
    .replace(/Function\s*\(/gi, 'FUNCTION_BLOCKED(') // Block Function constructor
    .replace(/setTimeout\s*\(/gi, 'SETTIMEOUT_BLOCKED(') // Block setTimeout
    .replace(/setInterval\s*\(/gi, 'SETINTERVAL_BLOCKED(') // Block setInterval
    .substring(0, 5000) // Limit code size
    .trim();
}
...
export async function reviewExerciseCode(
  ...
  try {
    // Sanitize all inputs to prevent AI injection
